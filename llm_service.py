...
def create_llm_service(provider: str = "gemini") -> Optional[LLMService]:
    """Factory function to create Gemini LLM service"""
    if provider.lower() != "gemini":
        print(f"Only Gemini is supported. Using Gemini instead of {provider}")
    
    # Use provided Gemini API key as fallback for deployment
    api_key = os.getenv("GEMINI_API_KEY", "AIzaSyDVwJ_jRxIgifFso36nGWaEOedJYYAuuU8")
...
def parse_images(images: List[str], preferred_provider: str = "gemini") -> Dict:
    """Standalone function for parsing images with Gemini"""
    
    service = create_llm_service("gemini")
    if service:
        try:
            return service._parse_with_gemini(images)
        except Exception as e:
